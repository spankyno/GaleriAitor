...
const Gallery: React.FC<GalleryProps> = ({ 
    ...
    return (
      <div className="h-full min-h-[60vh] flex flex-col items-center justify-center gap-8 animate-fade-in">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 font-bold animate-pulse">Conectando con Neon Database...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto animate-fade-in pb-24">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-10">
        <div>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-400 dark:text-slate-500">Exposición Activa</span>
          </div>
          ...
          <div className="flex items-center gap-3 mt-5">
            <div className="flex -space-x-2">
              {[1,2,3].map(i => <div key={i} className="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-800 border-2 border-white dark:border-slate-950"></div>)}
            </div>
            <p className="text-sm font-bold text-slate-600 dark:text-slate-300">
              <span className="text-blue-600 dark:text-blue-500 font-black">{images.length}</span> activos digitales encontrados
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5">
          <button 
            ...
            className="group p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-100 hover:border-blue-600 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-[1.5rem] transition-all shadow-sm active:scale-90"
          >
            {isDarkMode ? (
          ...
          <div className="flex items-center gap-1.5 bg-white dark:bg-slate-900 p-2 rounded-[1.75rem] shadow-sm border border-slate-200 dark:border-slate-800">
            {[
      ...
      {images.length === 0 ? (
        <div className="text-center py-40 bg-white/50 dark:bg-slate-900/30 rounded-[4rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
          <div className="w-28 h-28 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-10">
          ...
          {images.map((img, idx) => (
            <div 
              ...
              className={`
                ...
                ${viewMode === ViewMode.NORMAL ? 'rounded-[3rem] border border-slate-100 dark:border-slate-800 hover:shadow-2xl dark:hover:shadow-black hover:-translate-y-3' : ''}
                ...
                ${viewMode === ViewMode.LIST ? 'flex items-center gap-10 p-8 rounded-[3rem] hover:bg-white dark:hover:bg-slate-800/60 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 shadow-sm' : ''}
              `}
              ...
              `}>
                <div className={viewMode === ViewMode.LIST ? 'flex flex-col gap-2' : ''}>
                  ...
                  <p className={`text-slate-500 dark:text-slate-400 font-extrabold uppercase tracking-tight flex items-center gap-1.5 ${viewMode === ViewMode.MINIATURE ? 'text-[9px]' : 'text-[12px]'}`}>
                    <svg className="w-3 h-3 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>
                ...
                `}>
                  <button 
                    ...
                    className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-600 transition-all shadow-sm active:scale-90"
                    title="Copiar URL"
                  >
