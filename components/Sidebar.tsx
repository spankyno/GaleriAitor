...
const Sidebar: React.FC<SidebarProps> = ({ folders, currentFolder, onSelectFolder }) => {
  return (
    <div className="p-8 h-full flex flex-col bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl transition-colors">
      <div className="mb-14 hidden lg:block">
        ...
        <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400 mt-3 font-extrabold opacity-80">Portfolio Manager</p>
      </div>

      <nav className="flex-1 space-y-12">
        <section>
          <h2 className="text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-5 px-2">Visión General</h2>
          <button 
            ...
            className={`w-full group text-left px-5 py-4 rounded-[1.25rem] transition-all duration-300 text-sm flex items-center gap-4 font-bold ${
              ...
              : 'text-slate-600 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-900 hover:text-slate-950 dark:hover:text-white'
            }`}
          >
      ...
      <div className="mt-auto pt-8 border-t border-slate-100 dark:border-slate-900">
        <div className="bg-slate-50 dark:bg-slate-900/60 p-4 rounded-3xl flex items-center gap-4 border border-slate-100 dark:border-slate-800/50 shadow-sm">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-bold mt-0.5">Administrator</p>
          </div>
        </div>
      </div>
    </div>
